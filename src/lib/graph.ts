// graph.ts — shortest path with the thing every naive transit router forgets:
// changing trains costs real time, so the state is (station, line you're on),
// not just (station).

import type { City, Edge, Leg, RouteResult, Station } from './types.ts';
...
export function route(city: City, fromId: string, toId: string): RouteResult | null {
...
function toLegs(
  ...
  return {
    ...
    to: legs.length ? (legs[legs.length - 1]!.stations.slice(-1)[0] ?? null) : null,
  };
}
...
export function nearest(city: City, lat: number, lon: number): Nearest | undefined;
export function nearest(city: City, lat: number, lon: number, limit: number): Nearest[];
...
function dist(s: Station, lat: number, lon: number): number {
