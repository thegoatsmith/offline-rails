// data.js — where a city comes from and where it lives afterwards.
//
...
export const store = {
  saveCity: (city) => tx('cities', 'readwrite', (s) => s.put(city)),
  getCity: (id) => tx('cities', 'readonly', (s) => s.get(id)),
  allCities: () => tx('cities', 'readonly', (s) => s.getAll()),
  deleteCity: (id) => tx('cities', 'readwrite', (s) => s.delete(id)),
  setPref: (k, v) => tx('prefs', 'readwrite', (s) => s.put(v, k)),
  getPref: (k) => tx('prefs', 'readonly', (s) => s.get(k)),
};
...
export async function geocode(query) {
  ...
  return rows.map((r) => ({
    ...
    lat: +r.lat,
    lon: +r.lon,
    bbox: clampBox(r.boundingbox.map(Number), +r.lat, +r.lon),
  }));
}
...
function clampBox([south, north, west, east], lat, lon) {
...
export function emptyNetworkMessage(raw) {
...
const norm = (s) =>
  (s || '')
    ...
    .normalize('NFD')
    .replace(/[\u0300-\u036f]/g, '')
    .replace(/\b(station|stn|metro|subway|underground|bahnhof|gare|estacion|estación|stazione|станция|駅|站|역)\b/g, '')
    .replace(/[^\p{L}\p{N}]+/gu, ' ')
    .trim();

const SPEED = { subway: 11, light_rail: 9, tram: 6.5, monorail: 10, train: 16 };
const DWELL = 25;
...
function perpendicular(p, a, b, cosLat) {
  const px = (p[1] - a[1]) * cosLat * 111320, py = (p[0] - a[0]) * 110574;
  const bx = (b[1] - a[1]) * cosLat * 111320, by = (b[0] - a[0]) * 110574;
...
export function simplify(points, tolerance) {
  ...
  while (stack.length) {
    ...
    if (worst > tolerance && at > 0) { keep[at] = 1; stack.push([i, at], [at, j]); }
  }
  const out = [];
...
const compactShape = (points) => encodeShape(simplify(points, SIMPLIFY_TOLERANCE));
...
export const needsMigration = (city) => !(city.format >= CITY_FORMAT);

export function migrateCity(city) {
  if (city.format >= CITY_FORMAT) return false;
  for (const line of city.lines) line.shapes = line.shapes.map(compactShape);
...
function chainShapes(shapes) {
  ...
  usable.forEach((s, i) => { index(key(s[0]), i); index(key(s[s.length - 1]), i); });
  ...
  for (let i = 0; i < usable.length; i++) {
    ...
    for (;;) {
      ...
      used[j] = true;
      const s = usable[j];
      chain = chain.concat(key(s[0]) === k ? s.slice(1) : s.slice(0, -1).reverse());
    }
    for (;;) {
      const k = key(chain[0]);
      const j = take(k);
      ...
      used[j] = true;
      const s = usable[j];
      chain = (key(s[s.length - 1]) === k ? s.slice(0, -1) : s.slice(1).reverse()).concat(chain);
    }
    out.push(chain);
  }
  return out;
...
function boxTest(bbox) {
  if (!bbox || bbox.south == null || bbox.north == null) return null;
  return (lat, lon) => lat >= bbox.south && lat <= bbox.north && lon >= bbox.west && lon <= bbox.east;
}
...
function clipShapes(shapes, within) {
  if (!within) return shapes;
  const out = [];
  for (const shape of shapes) {
    let run = null;
    for (let i = 0; i < shape.length; i++) {
      if (within(shape[i][0], shape[i][1])) {
        if (!run) { run = []; if (i > 0) run.push(shape[i - 1]); }
        run.push(shape[i]);
      } else if (run) {
        run.push(shape[i]);
...
export function buildCity(raw, meta) {
  ...
  const stations = [];
  const byKey = new Map(); // normalised name -> station indices
  ...
  for (const rel of rels) {
    ...
    for (const s of stops) {
      if (!s.lines.some((l) => l.label === label && l.colour === line.colour)) {
...
function normaliseColour(c) {
  ...
  const named = {
    red: '#d64545', blue: '#3a72c4', green: '#3f9a5c', yellow: '#e0b530',
    orange: '#e08a30', purple: '#8a5bc4', brown: '#8a6a4a', grey: '#8b939e',
    gray: '#8b939e', black: '#41474f', white: '#c9ced6', pink: '#d96fa8',
    silver: '#a8adb5', gold: '#d4af37', cyan: '#3fb0c4', magenta: '#c44a9a',
  };
...
function buildGraph(stations, lines, byId) {
  const adj = {};
  ...
  for (const line of lines) {
    ...
    for (let i = 0; i < line.stops.length - 1; i++) {
      ...
      adj[a.id].push({ to: b.id, cost, line: line.id, ref: line.ref, colour: line.colour, mode: line.mode });
      adj[b.id].push({ to: a.id, cost, line: line.id, ref: line.ref, colour: line.colour, mode: line.mode });
    }
  }
